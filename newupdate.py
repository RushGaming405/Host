...
BOT_TOKEN = "8720387729:AAG_5OHgF7wlQK2TzBQHy-0gFkKq9jK4gjg"
TOKEN = BOT_TOKEN.strip()

if not TOKEN or TOKEN == "PASTE_YOUR_BOT_TOKEN_HERE":
    raise RuntimeError(
        "Please paste your BotFather token into BOT_TOKEN near the top of this file."
    )
OWNER_ID = int(os.getenv('OWNER_ID', 8520711928))
ADMIN_ID = int(os.getenv('ADMIN_ID', 8520711928))
YOUR_USERNAME = os.getenv('YOUR_USERNAME', '@73837')
UPDATE_CHANNEL = os.getenv('UPDATE_CHANNEL', '@74744747')
...
REQUIRED_MANDATORY_CHANNELS = {
    "-1004432072002": {
        "username": "@primedraco121",
        "name": "CYBER DRACO GC",
        "link": "https://t.me/primedraco121",
    },
    "-1003943385463": {
        "username": "@primedraco12",
        "name": "Cyber draco",
        "link": "https://t.me/primedraco12",
    },
}
...
init_db()
load_data()
for _channel_id, _channel_info in REQUIRED_MANDATORY_CHANNELS.items():
    mandatory_channels[_channel_id] = dict(_channel_info)
...
SECURITY_CONFIG = {
    ...
    "max_uncompressed_archive": 100 * 1024 * 1024,
    "max_script_runtime": 0,
    "max_memory_mb": 0,
    "max_processes": 0,
    "allowed_extensions": [".py", ".js", ".zip"],
    "blocked_imports": {
        "subprocess", "ctypes", "multiprocessing", "pickle", "marshal",
        "importlib", "pty", "popen", "winreg", "win32api", "win32process",
    },
    "blocked_calls": {
...
def _python_security_scan(content):
    reasons = []
    if ast is None:
        return False, ["Python AST unavailable"]
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            for alias in node.names:
                ...
                if root in SECURITY_CONFIG["blocked_imports"]:
            ...
            if isinstance(node.func, ast.Name) and node.func.id in SECURITY_CONFIG["blocked_calls"]:
                reasons.append(f"dangerous call: {node.func.id}()")
                ...
                if dotted in {
                    ...
                    "os.spawnv", "os.spawnve", "shutil.rmtree",
                    "subprocess.run", "subprocess.Popen", "subprocess.call",
                    "subprocess.check_output", "ctypes.CDLL",
                }:
                    reasons.append(f"dangerous call: {dotted}()")

    suspicious = [
        (r"curl\s+[^\\n|]*\|\s*(sh|bash)", "download-and-execute shell pipeline"),
        (r"wget\s+[^\\n|]*\|\s*(sh|bash)", "download-and-execute shell pipeline"),
        (r"/etc/(passwd|shadow)", "system credential file access"),
        (r"\.ssh/(id_rsa|authorized_keys|known_hosts)", "SSH credential access"),
        (r"rm\s+-rf\s+/(?:\s|$)", "destructive root deletion"),
        (r"(?:reverse|bind)\s*shell", "shell/backdoor indicator"),
        (r"base64\.b64decode\s*\(.*(?:exec|eval)", "obfuscated code execution"),
    ]
...
def _js_security_scan(content):
    reasons = []
    patterns = [
        (r"\bchild_process\b", "child_process access"),
        (r"\bexec(?:File|Sync)?\s*\(", "process execution"),
        (r"\bspawn(?:Sync)?\s*\(", "process spawning"),
        (r"\brequire\s*\(\s*['\"](?:child_process|vm|cluster)['\"]", "dangerous Node module"),
    ...
    try:
        if not content.strip():
            reasons.append("empty JavaScript file")
    except Exception:
        pass
    return (not reasons), reasons[:12]
...
def check_code_security(file_path, file_type):
    try:
        ...
        if size > SECURITY_CONFIG["max_file_size"]:
            return False, "file exceeds security size limit"

        with open(file_path, "r", encoding="utf-8", errors="ignore") as f:
            content = f.read()

        if file_type.lower() == "py":
            safe, reasons = _python_security_scan(content)
        elif file_type.lower() == "js":
            safe, reasons = _js_security_scan(content)
        else:
            return False, f"unsupported executable type: {file_type}"

        if not safe:
            logger.warning("SECURITY HOLD %s: %s", file_path, reasons)
            return False, "; ".join(reasons)
        return True, "Static security scan passed"
    except Exception as e:
        logger.exception("Security scan failed")
        return False, f"security scanner error: {e}"


def _zip_member_is_safe(name):
    if not name or name.startswith(("/", "\\")):
        return False
    normalized = os.path.normpath(name.replace("\\", "/"))
    return normalized not in (".", "") and not normalized.startswith("../") and "/../" not in f"/{normalized}"
...
def scan_zip_security(zip_path):
    try:
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            members = zf.infolist()
            if len(members) > SECURITY_CONFIG["max_archive_members"]:
                return False, f"archive has too many files ({len(members)})"
            ...
            for info in members:
                ...
                if mode == 0o120000:
                    return False, f"symlink entry is not allowed: {info.filename}"

                total_uncompressed += info.file_size
                if total_uncompressed > SECURITY_CONFIG["max_uncompressed_archive"]:
                    return False, "archive expands beyond 100 MB security limit"

                lower = info.filename.lower()
                if lower.endswith((".py", ".js")):
                    raw = zf.read(info)
                    if len(raw) > SECURITY_CONFIG["max_file_size"]:
                        return False, f"member too large: {info.filename}"
                    ...
                    if lower.endswith(".py"):
                        ...
                        safe, reasons = _js_security_scan(content)
                    if not safe:
                        return False, f"{info.filename}: " + "; ".join(reasons)

                if lower.endswith((".sh", ".bash", ".bat", ".cmd", ".ps1", ".exe", ".dll", ".so")):
                    return False, f"executable/installer file is not allowed: {info.filename}"

        return True, "Archive security scan passed"
    except zipfile.BadZipFile:
        return False, "invalid/corrupted ZIP archive"
    except Exception as e:
        logger.exception("ZIP security scan failed")
        return False, f"archive security scanner error: {e}"
...
def _sandbox_env(user_folder):
    ...
    own_token = os.getenv("BOT_TOKEN", "").strip()
    if own_token:
        for key in ("BOT_TOKEN", "TOKEN"):
            if env.get(key) == own_token:
                env.pop(key, None)
    return env


def _watch_script_runtime(script_key):
    limit = int(SECURITY_CONFIG.get("max_script_runtime", 0) or 0)
    if limit <= 0:
        return
    while script_key in bot_scripts:
        ...
        if not proc or proc.poll() is not None:
            return
        started = info.get("start_time", datetime.now())
        elapsed = (datetime.now() - started).total_seconds()
        if elapsed > limit:
            logger.warning("Runtime limit reached for %s; terminating PID %s", script_key, proc.pid)
            try:
...
def check_mandatory_subscription(user_id):
    ...
    not_joined = []
    channels_to_check = dict(REQUIRED_MANDATORY_CHANNELS)
    channels_to_check.update(mandatory_channels)
...
def get_user_running_bots_count(user_id):
    count = 0
...
def is_bot_running(script_owner_id, file_name):
    ...
    try:
        ...
        if not is_running:
            logger.warning(f"Process {process.pid} for {script_key} exited. Cleaning up.")
    ...
    try:
        if process and process.poll() is None:
            try:
                ...
                logger.info(f"Terminated process {process.pid} for {script_key}")
            except subprocess.TimeoutExpired:
                process.kill()
                process.wait(timeout=2)
                logger.info(f"Killed process {process.pid} for {script_key}")
            except ProcessLookupError:
                pass
        if log_file and hasattr(log_file, 'close') and not log_file.closed:
            log_file.close()
    except Exception as e:
        logger.error(f"Unexpected error stopping process for {script_key}: {e}", exc_info=True)
...
# ⭐ SAFE BUTTON HELPER
# ============================================================
def _safe_btn(text, callback_data=None, url=None, style=None):
    kwargs = {}
...
# ============================================================
# ⭐ SCREENSHOT-STYLE MAIN MENU (Saare Buttons, Sirf Inline)
# ============================================================
def create_main_menu_inline(user_id):
    """Screenshot jaisa design. Saare buttons (user + admin) intact."""
    markup = types.InlineKeyboardMarkup(row_width=2)

    if user_id in admin_ids:
        # ==================== ADMIN LAYOUT ====================
        # Row 1: Green - Main Action
        markup.row(_safe_btn('📤 UPLOAD FILE', callback_data='upload', style='success'))

        # Row 2: Blue 2-col
        markup.row(
            _safe_btn('📂 CHECK FILES', callback_data='check_files', style='primary'),
            _safe_btn('📊 STATISTICS', callback_data='stats', style='primary')
        )
        # Row 3: Blue 2-col
        markup.row(
            _safe_btn('⚡ BOT SPEED', callback_data='speed', style='primary'),
            _safe_btn('📦 MANUAL INSTALL', callback_data='manual_install', style='primary')
        )
        # Row 4: Blue 2-col - Admin
        markup.row(
            _safe_btn('💳 SUBSCRIPTIONS', callback_data='subscription', style='primary'),
            _safe_btn('📢 BROADCAST', callback_data='broadcast', style='primary')
        )
        # Row 5: Blue 2-col - Admin
        markup.row(
            _safe_btn('🔒 LOCK BOT' if not bot_locked else '🔓 UNLOCK BOT',
                      callback_data='lock_bot' if not bot_locked else 'unlock_bot', style='primary'),
            _safe_btn('🟢 RUNNING ALL CODE', callback_data='run_all_scripts', style='primary')
        )
        # Row 6: Blue 2-col - Admin
        markup.row(
            _safe_btn('👑 ADMIN PANEL', callback_data='admin_panel', style='primary'),
            _safe_btn('📢 CHANNEL ADD', callback_data='manage_mandatory_channels', style='primary')
        )
        # Row 7: Blue 2-col - Admin
        markup.row(
            _safe_btn('👥 USER MANAGEMENT', callback_data='user_management', style='primary'),
            _safe_btn('⚙️ SETTINGS', callback_data='admin_settings', style='primary')
        )
        # Row 8: Green full - Updates
        markup.row(_safe_btn('📢 UPDATES CHANNEL',
                             url=f'https://t.me/{UPDATE_CHANNEL.replace("@", "")}',
                             style='success'))
        # Row 9: Blue full - Contact
        markup.row(_safe_btn('📞 CONTACT OWNER',
                             url=f'https://t.me/{YOUR_USERNAME.replace("@", "")}',
                             style='primary'))
        # Row 10: Blue full - Help
        markup.row(_safe_btn('🆘 HELP / INFO', callback_data='help', style='primary'))
        ...
        # Row 4: Green full - Updates
        markup.row(_safe_btn('📢 UPDATES CHANNEL',
                             url=f'https://t.me/{UPDATE_CHANNEL.replace("@", "")}',
                             style='success'))
        # Row 5: Blue full - Contact
        markup.row(_safe_btn('📞 CONTACT OWNER',
                             url=f'https://t.me/{YOUR_USERNAME.replace("@", "")}',
                             style='primary'))
        # Row 6: Blue full - Help
        markup.row(_safe_btn('🆘 HELP / INFO', callback_data='help', style='primary'))
...
def create_admin_panel():
    ...
    markup.row(types.InlineKeyboardButton('🤖 All Users’ Hosted Bots', callback_data='admin_hosted_bots'))
    markup.row(types.InlineKeyboardButton('🔙 Back to Main', callback_data='back_to_main'))
    return markup
...
def handle_zip_file(downloaded_file_content, file_name_zip, message):
    ...
    try:
        ...
        scan_text = '✅ Static security scan passed' if is_safe else f'⚠️ Security warning: {security_msg}'
        security_warning_msg = (
            f"📦 NEW ZIP — ADMIN APPROVAL REQUIRED\n\n"
            f"👤 User: {user_id}\n"
            f"📁 File: {file_name_zip}\n"
            f"🔍 Scan: {scan_text}\n\n"
            f"⛔ ZIP will NOT be extracted, installed, hosted or started until an admin presses Approve."
        )
        markup = types.InlineKeyboardMarkup()
        markup.row(
            types.InlineKeyboardButton("✅ Approve", callback_data=f"approve_zip_{user_id}_{file_name_zip}"),
        ...
        bot.reply_to(message, "⏳ ZIP received. Admin approval is required before hosting/starting it.")
        return
...
def process_zip_file(zip_path, user_id, user_folder, file_name_zip, message, temp_dir=None):
    ...
    try:
        ...
        if req_file:
            ...
            try:
                command = [sys.executable, '-m', 'pip', 'install', '--disable-pip-version-check', '--no-input', '-r', req_path]
...
def _start_managed_process(cmd, cwd, log_file, env, file_type):
    startupinfo = None
    creationflags = 0
    if os.name == 'nt':
    ...
    return subprocess.Popen(
        cmd, cwd=cwd, stdout=log_file, stderr=log_file,
...
def _restart_managed_script(script_key):
    info = bot_scripts.get(script_key)
    if not info or info.get('stopping') or info.get('restart_thread_running'):
        return
    info['restart_thread_running'] = True
    ...
    def supervisor():
        ...
        try:
            while True:
                ...
                if not current or current.get('stopping'):
                    return
                ...
                logger.warning("Hosted process %s exited; restart in %ss (attempt %s)", script_key, delay, failures)
                try:
                    owner = current.get('script_owner_id')
                    if failures == 1 and owner:
                        bot.send_message(owner, f"⚠️ Hosted bot {current.get('file_name')} exited; restarting automatically.")
                except Exception:
                    pass
                time.sleep(delay)
                current = bot_scripts.get(script_key)
                ...
                try:
                    if log_file and not log_file.closed:
                        log_file.write(f"\n--- AUTO RESTART {datetime.now().isoformat()} ---\n")
                        log_file.flush()
                    cmd = [sys.executable, current['script_path']] if current.get('type') == 'py' else ['node', current['script_path']]
                    proc = _start_managed_process(cmd, current['user_folder'], log_file,
                                                  _sandbox_env(current['user_folder']), current.get('type'))
                    current['process'] = proc
                    current['pid'] = proc.pid
                    current['start_time'] = datetime.now()
                    logger.info("Auto-restarted %s with PID %s", script_key, proc.pid)
                    failures = 0
                except Exception:
                    logger.exception("Auto-restart failed for %s", script_key)
                    time.sleep(5)
        finally:
            current = bot_scripts.get(script_key)
            if current:
                current['restart_thread_running'] = False

    threading.Thread(target=supervisor, name=f"supervisor-{script_key}", daemon=True).start()
    ...
    try:
        ...
        if attempt == 1:
            ...
            try:
                check_proc = subprocess.Popen(
                    ...
                    text=True, encoding='utf-8', errors='ignore', env=_sandbox_env(user_folder),
                    start_new_session=(os.name != 'nt'),
        ...
        try:
            ...
            if os.name == 'nt':
                 ...
                 startupinfo.wShowWindow = subprocess.SW_HIDE
            process = _start_managed_process(
                [sys.executable, script_path], user_folder, log_file, _sandbox_env(user_folder), 'py'
            )
            ...
            bot_scripts[script_key] = {
                ...
                'script_path': script_path, 'pid': process.pid, 'stopping': False, 'restart_thread_running': False,
                'start_time': datetime.now(), 'user_folder': user_folder, 'type': 'py', 'script_key': script_key
            }
            threading.Thread(target=_watch_script_runtime, args=(script_key,), daemon=True).start()
            _restart_managed_script(script_key)
    ...
    try:
        ...
        try:
            ...
            if os.name == 'nt':
                 ...
                 startupinfo.wShowWindow = subprocess.SW_HIDE
            process = _start_managed_process(
                ['node', script_path], user_folder, log_file, _sandbox_env(user_folder), 'js'
            )
...
# ============================================================
# ⭐ WELCOME MESSAGE (Sirf Inline Buttons)
# ============================================================
def _logic_send_welcome(message):
    ...
    running_bots = get_user_running_bots_count(user_id)
    ...
    welcome_msg_text = (
        f"👤 ROLE      : {role_text}\n"
        f"💎 CREDITS   : {current_files} / {limit_str}\n"
        f"🔗 REFERRALS : {running_bots}\n"
        f"📊 USES      : {current_files}\n"
        f"🔥 APIS      : {current_files} FILE(S)\n"
        f"🔍 SCANNER   : {running_bots} DEVICE(S)\n\n"
        f"〽️ Welcome, {user_name}!\n\n"
        f"👇 TAP UPLOAD FILE TO START 📤"
    )

    main_inline_markup = create_main_menu_inline(user_id)
    try:
        bot.send_message(chat_id, welcome_msg_text, reply_markup=main_inline_markup)
    except Exception as e:
        logger.error(f"Error sending welcome to {user_id}: {e}", exc_info=True)
        try:
            bot.send_message(chat_id, welcome_msg_text)
        except Exception as e2:
            logger.error(f"Fallback also failed for {user_id}: {e2}")
...
PRIVATE_CHAT_CONTENT_TYPES = [
    "text", "audio", "document", "photo", "sticker", "video", "voice",
    "video_note", "location", "contact", "venue", "animation", "dice",
    "poll", "new_chat_members", "left_chat_member", "new_chat_title",
    "new_chat_photo", "delete_chat_photo", "group_chat_created",
    "supergroup_chat_created", "channel_chat_created", "migrate",
...
@bot.message_handler(
    ...
    content_types=PRIVATE_CHAT_CONTENT_TYPES
)
def ignore_non_private_messages(message):
    return
...
TEXT_COMMAND_TO_LOGIC = {
    "upload": _logic_upload_file,
    "checkfiles": _logic_check_files,
    "files": _logic_check_files,
    "botspeed": _logic_bot_speed,
    "speed": _logic_bot_speed,
    "contact": _logic_contact_owner,
    "stats": _logic_statistics,
    "statistics": _logic_statistics,
    "help": _logic_help,
    "info": _logic_help,
    "install": _logic_manual_install,
    "manual_install": _logic_manual_install,
    "adminpanel": _logic_admin_panel,
    "users": _logic_user_management,
    "usermanagement": _logic_user_management,
    "subscriptions": _logic_subscriptions_panel,
    "broadcast": _logic_broadcast_init,
    "settings": _logic_admin_settings,
    "channels": _logic_manage_mandatory_channels,
    "lockbot": _logic_toggle_lock_bot,
    "runall": _logic_run_all_scripts,
}

@bot.message_handler(func=lambda message: message.text and message.text.strip().lower().lstrip('/') in TEXT_COMMAND_TO_LOGIC and message.chat.type == "private")
def handle_text_command(message):
    key = message.text.strip().lower().lstrip('/')
    func = TEXT_COMMAND_TO_LOGIC.get(key)
    if func:
        func(message)
...
def handle_file_upload_doc(message):
    ...
    if current_files >= file_limit:
        limit_str = str(file_limit) if file_limit != float('inf') else "Unlimited"
        bot.reply_to(message, f"⚠️ File limit ({current_files}/{limit_str}) reached. Delete files via CHECK FILES.")
        return
    ...
    try:
        ...
        if file_ext == '.zip':
            ...
            pending_file_uploads[user_id][file_name] = {
                'content': downloaded_file_content,
                'type': file_ext[1:],
                'safe': is_safe,
                'security_msg': security_msg
            }
            status_text = '✅ Static security scan passed' if is_safe else f'⚠️ Security warning: {security_msg}'
            security_warning_msg = (
                f"📥 NEW FILE — ADMIN APPROVAL REQUIRED\n\n"
                f"👤 User: {user_id}\n"
                f"📁 File: {file_name}\n"
                f"🔍 Scan: {status_text}\n\n"
                f"⛔ File will NOT be hosted or started until an admin presses Approve."
            )
            markup = types.InlineKeyboardMarkup()
            markup.row(
            ...
            for admin_id in admin_ids:
                try:
                    ...
                    logger.error(f"Failed to send approval request to admin {admin_id}: {e}")
            bot.reply_to(message, "⏳ File received. Admin approval is required before hosting/starting it.")
            return
...
def handle_callbacks(call):
    ...
    if is_user_banned(user_id) and data not in ['back_to_main']:
        ...
        return
    if data not in ['check_subscription_status', 'back_to_main', 'manual_install', 'help']:
        is_subscribed, not_joined = check_mandatory_subscription(user_id)
        if not is_subscribed and user_id not in admin_ids:
            subscription_message, markup = create_subscription_check_message(not_joined)
            bot.answer_callback_query(call.id)
            ...
            return
    if bot_locked and user_id not in admin_ids and data not in ['back_to_main', 'speed', 'stats', 'check_subscription_status', 'manual_install', 'help']:
    ...
    try:
        ...
        elif data == 'admin_hosted_bots': admin_required_callback(call, admin_hosted_bots_callback)
...
def stop_bot_callback(call):
    try:
        ...
        else: logger.warning(f"Script {script_key} running by process check but not in bot_scripts dict.")
        try:
            bot.edit_message_text(
                f"⚙️ Controls for: `{file_name}` ({file_type}) of User `{script_owner_id}`\nStatus: 🔴 Stopped",
                chat_id_for_reply, call.message.message_id,
...
def back_to_main_callback(call):
    ...
    main_menu_text = (
        ...
        f"🔥 APIS      : {current_files} FILE(S)\n"
        f"🔍 SCANNER   : {running_bots} DEVICE(S)\n\n"
        f"〽️ Welcome back, {call.from_user.first_name}!\n\n"
        f"👇 TAP UPLOAD FILE TO START 📤"
    )
    try:
        bot.answer_callback_query(call.id)
...
def admin_hosted_bots_callback(call):
    if call.from_user.id not in admin_ids:
    ...
    markup = types.InlineKeyboardMarkup(row_width=1)
    count = 0
    for owner_id in sorted(user_files):
        for file_name, file_type in sorted(user_files.get(owner_id, [])):
            running = is_bot_running(owner_id, file_name)
            status = "🟢 Running" if running else "🔴 Stopped"
            label = f"{status} | {owner_id} | {file_name}"
            markup.add(types.InlineKeyboardButton(label[:60], callback_data=f"file_{owner_id}_{file_name}"))
            count += 1
    markup.add(types.InlineKeyboardButton("🔄 Refresh", callback_data="admin_hosted_bots"))
    markup.add(types.InlineKeyboardButton("🔙 Admin Panel", callback_data="admin_panel"))
    text = f"🤖 All users’ hosted bots\nTotal registered: {count}\nSelect a bot to open Start/Stop/Restart controls."
    try:
...
def system_info_callback(call):
    ...
    try:
        ...
        info_parts.append(f"• Bot Uptime: {time.strftime('%H:%M:%S', time.gmtime(time.time() - BOT_START_TIME))}")
        info_parts.append("\n📊 **Bot Statistics:**")
...
def process_approve_file(call):
    ...
    pending = pending_file_uploads.get(user_id, {}).get(file_name)
    if not pending:
        bot.answer_callback_query(call.id, "❌ Pending file not found. Ask user to re-upload.", show_alert=True)
        return
    user_folder = get_user_folder(user_id)
    file_path = os.path.join(user_folder, file_name)
    file_ext = os.path.splitext(file_name)[1].lower()
    try:
        ...
        del pending_file_uploads[user_id][file_name]
        if not pending_file_uploads[user_id]:
            del pending_file_uploads[user_id]
        bot.answer_callback_query(call.id, "✅ Approved and started.")
        bot.edit_message_text(
            f"✅ File `{file_name}` approved by admin for user `{user_id}` and started.",
            call.message.chat.id, call.message.message_id, parse_mode='Markdown'
        )
        try:
            bot.send_message(user_id, f"✅ Your file `{file_name}` was approved by admin and has been started.")
        except Exception as e:
            logger.error(f"Failed to notify user {user_id}: {e}")
    except Exception as e:
        logger.error(f"Error processing approved file: {e}", exc_info=True)
        bot.answer_callback_query(call.id, "❌ Error starting approved file.", show_alert=True)
...
def process_reject_file(call):
    ...
    try:
        ...
        return
    file_name = '_'.join(data_parts[3:])
    pending = pending_file_uploads.get(user_id, {}).pop(file_name, None)
    if user_id in pending_file_uploads and not pending_file_uploads[user_id]:
        del pending_file_uploads[user_id]
    user_folder = get_user_folder(user_id)
    file_path = os.path.join(user_folder, file_name)
    if os.path.exists(file_path):
    ...
    try:
        bot.edit_message_text(
            f"❌ File `{file_name}` rejected for user `{user_id}`.",
            call.message.chat.id, call.message.message_id, parse_mode='Markdown'
        )
    except Exception:
        pass
...
# --- Telegram polling/network configuration ---
POLL_HTTP_TIMEOUT = int(os.getenv("POLL_HTTP_TIMEOUT", "90"))
POLL_LONG_TIMEOUT = int(os.getenv("POLL_LONG_TIMEOUT", "60"))
POLL_RETRY_DELAY = int(os.getenv("POLL_RETRY_DELAY", "5"))
POLL_CONNECTION_RETRY_DELAY = int(os.getenv("POLL_CONNECTION_RETRY_DELAY", "10"))
POLL_ERROR_RETRY_DELAY = int(os.getenv("POLL_ERROR_RETRY_DELAY", "15"))

# --- Startup validation ---
def validate_mandatory_destinations():
    for channel_id, info in REQUIRED_MANDATORY_CHANNELS.items():
        try:
            chat = bot.get_chat(channel_id)
            logger.info("Mandatory destination OK: %s (%s)", info.get("name"), getattr(chat, "type", "unknown"))
        except Exception as e:
            logger.error(
                "Mandatory destination %s is not reachable. "
                "Make sure the bot is a member/admin there and the ID/username is correct: %s",
                info.get("name"), e
            )
...
if __name__ == '__main__':
    ...
    keep_alive()
    validate_mandatory_destinations()
    ...
    while True:
        try:
            bot.infinity_polling(
                ...
                long_polling_timeout=POLL_LONG_TIMEOUT,
                skip_pending=True,
                allowed_updates=None
            )

        except requests.exceptions.ReadTimeout as e:
            logger.warning(f"Polling ReadTimeout: {e!s}. Retrying in {POLL_RETRY_DELAY}s...")
            time.sleep(POLL_RETRY_DELAY)
        except requests.exceptions.ConnectTimeout as e:
            logger.warning(f"Polling ConnectTimeout: {e!s}. Retrying in {POLL_RETRY_DELAY}s...")
            time.sleep(POLL_RETRY_DELAY)
        except requests.exceptions.ConnectionError as e:
            logger.warning(f"Polling ConnectionError: {e!s}. Retrying in {POLL_CONNECTION_RETRY_DELAY}s...")
            time.sleep(POLL_CONNECTION_RETRY_DELAY)
        except requests.exceptions.RequestException as e:
            logger.warning(f"Telegram HTTP error: {e!s}. Retrying in {POLL_RETRY_DELAY}s...")
            time.sleep(POLL_RETRY_DELAY)
        except KeyboardInterrupt:
            logger.info("🛑 Bot stopped by user.")
            break
        except Exception as e:
            logger.critical(f"💥 Polling error: {e!s}. Restarting in {POLL_ERROR_RETRY_DELAY}s...", exc_info=True)
            time.sleep(POLL_ERROR_RETRY_DELAY)
        finally:
            logger.info("Polling cycle ended; watchdog loop remains active.")
