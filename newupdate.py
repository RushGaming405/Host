...
TOKEN = '8866124186:AAF7o0iFCvpS8P0iQsIS4mLoOEGcboqK3IQ'
OWNER_ID = 8520711928
ADMIN_ID = 8520711928
YOUR_USERNAME = '@optimusprime50'
UPDATE_CHANNEL = 'https://t.me/primedraco12'

# ✅ FORCE JOIN CHANNEL CONFIG
FORCE_JOIN_ENABLED = True
CHANNEL_ID = -1003943385463  # Replace with your channel ID (get from @userinfobot)
CHANNEL_USERNAME = 'primedraco12'  # Your channel username
...
def force_join_channel(user_id):
    """Attempt to add user to channel, return True if successful or already member"""
    try:
        bot.add_chat_members(CHANNEL_ID, [user_id])
        logger.info(f"User {user_id} force-added to channel {CHANNEL_USERNAME}")
        return True
    except Exception as e:
        error_msg = str(e).lower()
        if "already" in error_msg or "participant" in error_msg:
            logger.info(f"User {user_id} already in channel")
            return True
        elif "privacy" in error_msg or "restricted" in error_msg:
            logger.warning(f"User {user_id} privacy restricted - manual join required")
            return False
        else:
            logger.warning(f"Force join failed for {user_id}: {e}")
            return False
...
def verify_channel_access(message):
    """Verify and enforce channel membership. Returns True if user has access."""
    ...
    # Try to force add
    if force_join_channel(user_id):
        return True
    
    # If force join fails, ask them to join manually
    join_msg = (
    ...
    bot.send_message(message.chat.id, join_msg, parse_mode='Markdown')
    logger.warning(f"User {user_id} denied access - not in channel and force join failed")
    return False
...
def _logic_send_welcome(message):
    ...
    if not verify_channel_access(message):
        return
...
def _logic_run_all_scripts(message_or_call):
    if isinstance(message_or_call, telebot.types.Message):
        ...
        # ✅ FORCE CHANNEL JOIN CHECK
        if not verify_channel_access(message_or_call):
            return
